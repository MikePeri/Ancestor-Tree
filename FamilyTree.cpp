...
namespace family{
	...
	string Tree::relation(string person){
		...
		int deaph=findDeaph(this->root,person,0);
	 ...
	 int Tree::findDeaph(node* root,string name,int level)
	 {
		if(root==NULL)
			return 0;
		if(root->name==name)
		 	return level;
		int deaph=findDeaph(root->father,name,level+1);
		if(deaph==0)
			deaph=findDeaph(root->mother,name,level+1);
		return deaph;
